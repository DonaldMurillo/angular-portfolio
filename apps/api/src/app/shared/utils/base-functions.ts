...
export async function tryAndSaveEntity<T>(item: DeepPartial<T>, repository: Repository<T>): Promise<T> {
	try {
...
export function tryAndSaveEntityError(error: any) {
	...
	if (!postgresError?.code) {
		...
		const message = postgresError.detail?.split('=')[1].replace('(', '"').replace(')', '"');
		...
		throw new NotAcceptableException(`Property "${messageArr[1]}" cannot be null`);
	} else {
