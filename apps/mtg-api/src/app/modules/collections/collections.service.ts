...
export class CollectionsService {
  ...
  async create(user: User, createCollectionDto: CreateCollectionDto) {
    const userProfile: UserProfile = await this.profileService.findOneByUser(
      user
    );
    if (!userProfile || userProfile.id !== createCollectionDto.profileId) {
      throw new UnauthorizedException();
    }
    const existingCollection = await this.collectionRepository.find({
      where: { profile: userProfile, name: createCollectionDto.name },
    });
    ...
    const { profile, ...newCollection } = await tryAndSaveEntity(
      collection,
  ...
  async userCollections(user: User) {
    ...
    const collections = this.collectionRepository.find({
  ...
  async findOne(user: User, id: string): Promise<[Collection, UserProfile]> {
    ...
    const existCollection = userProfile.collections.find(
      (col) => col.id === id
    );
    if (!existCollection) {
  ...
  async update(
    ...
    if (updateCollectionDto.name) {
      const namedCollection = userProfile.collections.find(
        (col) => col.name === updateCollectionDto.name
      );
  ...
  async createCollectionItem(
    ...
    const existItem = existCollection.items.find(
      (col) => col.scryfallId === createCollectionItemDto.scryfallId
    );
    if (existItem) {
      // IF EXISTS INCREMENT QUANTITY
      return await tryAndSaveEntity(
    ...
    const { collection, ...newItem } = await tryAndSaveEntity(
      item,
      this.collectionItemRepository
    );
  ...
  async updateCollectionItem(
    ...
    const { collection, ...updatedItem } = await tryAndSaveEntity(
  ...
  async share(userNickname: string, collectionName: string) {
    ...
    const sharedUserProfile: UserProfile =
      await this.userProfileRepository.findOne({ nickname: userNickname });
    if (!sharedUserProfile) {
      throw new NotFoundException();
    }

    //verify collection exist, and also, it is shareable
    const collection = sharedUserProfile.collections.find(
      (col) => col.name === collectionName && col.tradeable === true
    );
    if (!collection) {
      throw new NotFoundException();
    }
