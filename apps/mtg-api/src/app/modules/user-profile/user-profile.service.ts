...
export class UserProfileService {
  ...
  async create(createUserProfileDto: CreateUserProfileDto, user: User) {
    ...
    const { user: myUser, ...profile } = await tryAndSaveEntity(
      userProfile,
  ...
  async update(user: User, updateUserProfileDto: UpdateUserProfileDto) {
    ...
    if (
      ...
      updateUserProfileDto.nickname !== userProfile.nickname
    ) {
      const checkProfile = await this.userProfileRepository.findOne({
        where: { nickname: updateUserProfileDto.nickname },
      });
      if (checkProfile) throw new NotAcceptableException();
    }

    const newUserProfile = this.userProfileRepository.merge(
      userProfile,
